        ...
        do{
                ...
                strcpy(object,roomInfo);
                 ...
                 if(object[0] == 'd'){
                    ...
                    if(object[1] == 'e'){
                        mvaddch(doorPos-1,roomX, DOOR);
                    }
                    if(object[1] == 'w'){
                        mvaddch(doorPos-1,0, DOOR);
                    }
                    if(object[1] == 'n'){
...
void printMessage (char * s, int lineNum, int xDir)
{
    ...
    /*prints the message on the line indicated plus 33 lines so not to interfere with gameboard*/
    mvwprintw (stdscr, 33+lineNum, xDir, s );
...
void printItems (int lineNum, struct Items * hero)
{
    ...
    mvprintw (0, 33, "******* ROGUE *******");
    mvprintw (1, 33, "i = up       q = quit");
    mvprintw (2, 33, "k = down     n = teleport");
    mvprintw (3, 33, "j = left ");
    mvprintw (4, 33, "l = right");
    mvprintw (7, 33, "Items        Enemies");
    mvprintw (9, 33, "* = gold     M = monster");
    mvprintw (10, 33,"! = potion");
    mvprintw (11, 33,") = weapon");

    /*prints the heros items on the line indicated plus 30 lines so not to interfere with gameboard*/
    mvprintw (16+lineNum, 33, "Gold: %d ", hero->treasure);
    mvprintw (17+lineNum, 33, "Nazerath Potions: %d ", hero->potion);
    mvprintw (18+lineNum, 33, "Weapons(Kobold Swords): %d ", hero->weapon);
